...
class TestInfoConservation:
    ...
    def test_check_one_zero_opp_data_amount(self,
                                            out_file_path=os.path.join(os.path.dirname(os.getcwd()), out_filename)):
        df = pd.read_csv(out_file_path)
    ...
    def test_check_memory_overflow_satellite(self,
                                             out_file_path=os.path.join(os.path.dirname(os.getcwd()), out_filename),
        ...
        for _, el in df.iterrows():
            ...
            assert sattelites_data_dict[
                       el['origin'][-14:]] <= max_cap, f'Sattelite memory overflowed!\n\ndict: {sattelites_data_dict}'

    def test_check_proper_sign_for_downlink_and_imaging(self, out_file_path=os.path.join(os.path.dirname(os.getcwd()),
                                                                                         out_filename)):
        df = pd.read_csv(out_file_path)
